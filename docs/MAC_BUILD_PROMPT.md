...
1. Get this repo onto the Mac (it has ssh). From the Mac, pull it off the
   Windows box, e.g.:
       rsync -avz --exclude node_modules --exclude bin --exclude obj \
             <winuser>@<windows-host>:/c/Users/<youruser>/agent-omega/  ~/agent-omega/
...
1. **Confirm you're inside the repo.** You should see `README.md`, `TECHNICAL.md`, `Program.cs`, `sidecar.mjs`, `ui/`, `config-template/`, and `docs/`. If not, get the repo first — it lives at `~/agent-omega` (or `C:/Users/<youruser>/agent-omega`) on the Windows box; pull it over SSH (`rsync`/`scp`) or clone it.
