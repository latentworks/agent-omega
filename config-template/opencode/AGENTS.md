...
A defining capability of this tool: you can hand self-contained subtasks to worker subagents with the `task` tool for fast PARALLEL grunt work — `helper1` and `helper2` (run them alongside each other when you want two subtasks going at once). They point at your **local** model (configure the `local` provider in `opencode.json` — see SETUP.md), so this matters most when you (the lead) are a cloud model handing heavy, mechanical, or private work down to a local worker. If no local worker is configured, just do the work inline. Use them well:
