...
pass(exists('vault/index.mjs') && exists('vault/linux-env.mjs') && exists('vault/file-vault.mjs') && exists('vault/windows-dpapi.mjs'), 'Linux vault backend must exist')
...
pass(/defaultEnginePath/.test(sidecar) && /opencode/.test(sidecar) && /opencode\.exe/.test(sidecar), 'sidecar.mjs must use a platform engine resolver')
pass(!/execFileSync\(['"]powershell['"]/.test(sidecar), 'sidecar.mjs must not call PowerShell directly')
