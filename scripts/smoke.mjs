...
// 3) Vault script present (installed or self-heal source). Backend is per-OS:
//    Windows = scripts/secrets.ps1 (DPAPI), macOS/other = mac/secrets.sh (Keychain).
{
  const isWin = process.platform === 'win32'
  const vaultName = isWin ? 'secrets.ps1' : 'secrets.sh'
  const vault = process.env.AGENT_OMEGA_VAULT || path.join(HOME, '.agent-omega', vaultName)
  const src = isWin ? path.join(REPO, 'scripts', 'secrets.ps1') : path.join(REPO, 'mac', 'secrets.sh')
  if (existsSync(vault)) pass('vault script installed -> ' + vault)
  else if (existsSync(src)) warn('vault not installed yet, but source exists (' + src + ')')
