...
- Download **`opencode-windows-x64.exe`** and **`SHA256SUMS-omega-v2.7.3.txt`** from the [Agent Omega engine v2.7.3 release](https://github.com/latentworks/opencode-omega/releases/tag/omega-v2.7.3). Rename the binary to `opencode.exe`. This exact forked engine is required; upstream opencode and earlier Agent Omega engines are intentionally rejected at startup.
...
- **Qwen3-Coder-Next 80B stalls on AMD Vulkan/RADV** — disable the five speculative-decoding flags documented in [Model Tunes](TUNES.md#qwen3-coder-next-80b-on-amd-vulkanradv). Keep main-model GPU offload (`-ngl`) distinct from draft-model offload (`-ngld`).
